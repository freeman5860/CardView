...
public class BusinessCardViewScroller {
	...
	BusinessCardViewLayoutAlgorithm<?> mLayoutAlgorithm;
	...
	public BusinessCardViewScroller(Context context,
			BusinessCardViewConfig config,
	...
	/**
	 * Sets the current stack scroll to the initial state when you first enter
	 * recents.
	 * 
	...
	/**
	 * Bounds the current scroll if necessary, but does not synchronize the
	 * stack view with the model.
	 */
	...
	/**
	 * Returns the amount that the aboslute value of how much the scroll is out
	 * of bounds.
	 */
		...
		if (DVUtils.isAboveSDKVersion(11)) {
			...
			if (mScrollAnimator != null && mScrollAnimator.isRunning()) {
				...
				mScroller.startScroll(0,
						progressToScrollRange(mFinalAnimatedScroll), 0, 0, 0);
			}
			stopScroller();
			stopBoundScrollAnimation();

			mFinalAnimatedScroll = newScroll;
			mScrollAnimator = ObjectAnimator.ofFloat(this, "stackScroll",
					curScroll, newScroll);
			...
			animateScrollCompat(curScroll, newScroll, postRunnable);
		}
	}

	void animateScrollCompat(float curScroll, float newScroll,
			final Runnable postRunnable) {
		mFinalAnimatedScroll = newScroll;
		setStackScroll(mFinalAnimatedScroll);
		mScroller.startScroll(0, progressToScrollRange(mFinalAnimatedScroll),
				0, 0, 0);
	}
